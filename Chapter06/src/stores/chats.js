...
class Chats {
  addMessages = function(chatId, contactId, messages) {
  ...
  add(user1, user2) {
    return new Promise(function(resolve, reject) {
      firebaseApp
        .database()
        .ref("/chats/" + user1.id + "/" + user1.id + user2.id)
        .set({
          name: user2.name,
          image: user2.avatar,
          contactId: user2.id
        ...
        .then(() => {
          firebaseApp
            .database()
            .ref("/chats/" + user2.id + "/" + user1.id + user2.id)
            .set({
              name: user1.name,
              image: user1.avatar,
              contactId: user1.id
...
decorate(Chats, {
  list: observable,
  selectedChatMessages: observable,
  downloadingChats: observable,
  downloadingChat: observable
});
